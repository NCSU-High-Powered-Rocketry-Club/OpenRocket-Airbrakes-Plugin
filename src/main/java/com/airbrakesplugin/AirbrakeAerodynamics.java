...
public class AirbrakeAerodynamics {
    ...
    public double getIncrementalCd(double mach, double deployFrac) {
        try {
            double clampedMach = clamp(mach, machPoints[0], machPoints[machPoints.length - 1]);
            double clampedDeploy = clamp(deployFrac, deploymentPoints[0], deploymentPoints[deploymentPoints.length - 1]);
            return cdFunction.value(clampedMach, clampedDeploy);
        } catch (Exception e) {
            LOG.warn("Cd interpolation failed: mach={}, deploy={}, error={}", 
    ...
    public double getIncrementalCm(double mach, double deployFrac) {
        try {
            double clampedMach = clamp(mach, machPoints[0], machPoints[machPoints.length - 1]);
            double clampedDeploy = clamp(deployFrac, deploymentPoints[0], deploymentPoints[deploymentPoints.length - 1]);
            return cmFunction.value(clampedMach, clampedDeploy);
        } catch (Exception e) {
            LOG.warn("Cm interpolation failed: mach={}, deploy={}, error={}", 
                    mach, deployFrac, e.getMessage());
            return 0.0;
        }
    }
