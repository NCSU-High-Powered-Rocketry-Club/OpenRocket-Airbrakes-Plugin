...
//         // Seed flight data so postAerodynamicCalculation never sees NaN on first read
...
//         double airbrakeExt = status.getFlightDataBranch().getLast(AIRBRAKE_EXT);
//         if (!Double.isFinite(airbrakeExt)) {
//             log.debug("airbrakeExt from FlightDataBranch is NaN; falling back to internal ext={}", ext);
//             airbrakeExt = ext;  // ext is always a finite 0.0 or 1.0 via ctx callbacks
//         }

//         // NaN-safe early-out: !(x > 1e-6) is true for x <= 1e-6 or NaN
//         if (!(airbrakeExt > 1e-6)) {
